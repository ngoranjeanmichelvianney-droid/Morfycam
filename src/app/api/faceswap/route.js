...
export async function connecterDecart(
  ...
  const { modelId = "lucy-2.5" } = options;

  // Un nouveau token éphémère à chaque connexion (il expire après ~10 min,
  // mais une session déjà connectée continue de fonctionner après expiration).
  ...
  const realtimeClient = await client.realtime.connect(stream, {
    ...
    initialState: {
      prompt: {
        text: "Swap the person's face with the reference avatar image",
        image: avatarUrl,
      },
    },
  });
...
/** Remplace atomiquement prompt + image de référence. */
export function mettreAJourAvatar(realtimeClient, avatarUrl, texte) {
  return realtimeClient?.set?.({
    prompt: texte ?? "Swap the person's face with the reference avatar image",
    image: avatarUrl,
  });
}
